...
sub extract_app_info {
    ...
    eval {
        ...
        if ( $download_html =~ m{<span>(.*?)</span>} ) {
            my $size = $1;
            $app_info->{size} = kb_m($size) if $size =~ /\d/;
        }
        ...
        $app_info->{description} = del_inline_elements($desc->as_HTML);
        ...
        $cookie_jar->set_cookie( undef, "mobile_phone_url", "/htc/htc_desire",
            "/mobile/", "www.brothersoft.com", 80 );
        $cookie_jar->set_cookie( undef, "sub_device", "2015", "/",
            "m.brothersoft.com", 80 );
        ...
        if ( $downloader->is_success ) {
            if ( $brothersoft_info =~
                m{<img src="([^"]+)"[^>]+?class="thumb"/>}s )
            {
                $app_info->{icon} = $1;
            }
            if ( $brothersoft_info =~
                m{<strong>Downloads:</strong>.*?([\d,]+)}s )
            {
                my $tmp_times = trim($1);
                $tmp_times =~ s/,//g;
