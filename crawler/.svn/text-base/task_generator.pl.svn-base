...
{		
    ...
    while(my $ip_hash=$ip_sth->fetchrow_hashref){
        ...
        $sql = "select SQL_BUFFER_RESULT app_url_md5,app_url from app_info where ".
            " market_id=$market->{'id'}  and worker_ip='$ip' and ".
            " (status!='invalid' or ".
            "   (status='invalid' and TIMESTAMPDIFF( MINUTE,last_visited_time, now() )/1440>7))".            " order by last_visited_time, app_url_md5";  
        my $sth = $dbh->prepare($sql);
        $sth->execute();
...
{		
    ...
    my $sql = "select SQL_BUFFER_RESULT app_url_md5, app_url from app_source where ".
        " (status='undo' or 
            (status='fail' and TIMESTAMPDIFF( MINUTE,last_visited_time, now() )/60 >24 ) or
            (status='invalid' and TIMESTAMPDIFF(MINUTE,last_visited_time, now())/60 >168) ) ".
