...
our %app_map_func = (
        author                  => sub {
            ...
            $log->('warn' => "can't find author") and return 'unknown' unless @nodes;
            return 
        ...
        current_version         => sub{
            ...
            if( (my $text = $nodes[0]->as_text) =~ m/([\d\.]+)/ ){
                return $1
            }else{
                return 0
            }
        ...
        official_rating_times   => sub {
            my ( $html,$app_info ) = ( shift,pop );
            my $node = $tree->look_down( class => 'total-votes' );
            return 0 unless ref($node);
            return ($node->find_by_tag_name('span'))[1]->as_text;
        },
        app_qr                  => sub{
            my ( $html,$app_info ) = ( shift,pop );
            my $bar = $tree->look_down( class => 'barcode' );
            $log->( warn => 'not find qrcode') unless ref $bar;
        ...
        official_comment_times  => sub {
            my ( $html,$app_info ) = ( shift,pop );
            my $node = $tree->look_down( class => 'comments' );
            return 0 unless ref($node);
            return $node->find_by_tag_name('a')->as_text;
        },
        description             => sub{
            my ( $html,$app_info ) = ( shift,pop );
            my $node = $tree->look_down( class => qr/node node-mobileapp/ );
            $log->(warn => 'find desc failed') unless ref($node);
        ...
        official_category       => sub {
            ...
            $log->( warn => 'not find category of app' ) unless @nodes;
        ...
        price                   => sub {
            ...
            $log->( warn => 'not find price of app' ) unless ref($tag);
            my $c = $tag->as_text;
            if( $c =~ m/\$([\d\.]+)/ ){
                return 'USD:'.$1
            }
        ...
        permission              => sub{
            ...
            $log->( warn => 'not find permission') and return unless ref $node;
...
sub extract_page_list{
    ...
    eval{
        ...
        $log->( 'warn' => 'find last page failed') unless ref($node);
