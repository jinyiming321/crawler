...
sub extract_app_info {
    ...
    eval {
        ...
        if (   not defined( $app_info->{app_name} )
            || not defined( $app_info->{current_version} ) )
        {
...
/软件性质：(.*?)软件标签：(.*?)发布时间：(.*?)格式\/大小：(.*?)软件类型：(.*?)适用系统：(.*?)上传者：(.*?)适用型号：(.*?)和/m
          )
        {
            ...
            $app_info->{size}        = int( $1 * 1024 )
              if $matched{format_size} =~ /^\D+(\d+).*/;
        ...
        if ( scalar @a_tag ) {
            ...
            $app_info->{apk_url} =
              URI->new( $a_tag[0]->attr("href") )
              ->abs( dirname( $app_info->{app_url} ) )->as_string;
        }

        #description
        @nodes = $tree->look_down( class => "software-b1" );
        $app_info->{description} = del_inline_elements($nodes[0]->as_HTML)
          if scalar @nodes;
        ...
        @tags = $tree->look_down(
            "_tag", "img",
            sub {
                defined( $_[0]->attr("onload") )
                  && $_[0]->attr("onload") =~ /javascript/;
            }
        );
...
sub extract_page_list {
    ...
    eval {
        ...
        if ( $page_node->attr("onclick") =~ /WebPages\(([^)]+)\)/ ) {
            ...
            push @$pages,
              map { $base_url_real . "/" . $pre . $mid . "-" . $_ . ".html" }
              1 .. $post;
        }
    };
