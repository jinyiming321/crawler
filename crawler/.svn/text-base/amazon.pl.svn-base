...
#$webpage =~ m!<span id="btAsinTitle" style="">(.*?)</span>!g;
    ...
    $app_info->{price}=0;
    if ($webpage =~ m!<b class="priceLarge">\$([\d\.]+)</b>!) {
        $app_info->{price}="USD:$1";
        $app_info->{currency}="USD";
    }
...
sub download_app_apk 
{
    ...
    my $asin=$apk_info->{apk_url};
    my $url= &get_download_url($apk_info);
    unless( defined $url)
    {
        $apk_info->{'status'}='fail';
        return 0;
    }
    print("end get download url for $asin\n");

    print("start download apk for $asin\n");
    my $unique_name = &download_apk($apk_dir,$url, $asin) ;
    unless (defined $unique_name)
    {
        $apk_info->{'status'}='fail';
        return 0;
    }
...
sub get_download_url{
    ...
    my $hash_ref=$dbh->selectrow_hashref("select amazon_interval_version from app_extra_info where app_url_md5='$apk_info->{app_url_md5}'");
    my $app_internal_version=$hash_ref->{'amazon_interval_version'};
    
    my $cmd ="java -jar ".$conf->getAttribute('BaseBinDir')."/amazon/amazonDown.jar ";
    
    $cmd .= " -a $app_asin" ;
    $cmd .= " -v $app_internal_version" if defined $app_internal_version;
    ...
    $dbh->do("update app_extra_info set amazon_interval_version='$app_internal_version' where app_url_md5='$apk_info->{app_url_md5}'");  
    return $download_url;
}
...
sub download_apk {
    ...
    do{
        ...
        unless ($response->is_success){
            $logger->error("fail to get APK:$asin");
            warn "fail to get apk for $asin";
            $logger->error("fail to get apk for $asin, error code:403") if $response->code==403;
            sleep(60*$retry);
        }
...
sub kill_java{
    ...
    my $cmd="pid=`ps axu |grep $asin|grep -v grep |awk '{print \$2}'`; kill \$pid";
    system($cmd);
}
