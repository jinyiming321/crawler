...
sub extract_app_info {
    ...
    eval {
        ...
        $app_info->{description} = del_inline_elements($en_desc->as_HTML) if ref $en_desc;
        if ( not defined( $app_info->{description} ) ) {
            my $des = $tree->look_down( id => "softwareDetailDescription" );
            $app_info->{description} = del_inline_elements($des->as_HTML);
        }
...
m{Uploaded: (.*?)Updated: (.*?)Version: (.*?)Downloads: (\d+)Category: (.*?)Views: (\d+)License: (.*?) (?:Company Name|Developer:)(.*)}s
          )
        {
            ...
            $app_info->{current_version} = $1
              if $app_info->{current_version} =~
                  /^(?:[vV]?\.?(?:ersion )?)([.\d]+)[abc]?$/;
        ...
        #comment_times
        $app_info->{official_comment_times} = 0;    #default 0
        my @comment_nodes = $tree->look_down( class => 'comments-review-box' );
        $app_info->{official_comment_times} = scalar(@comment_nodes)
          if @comment_nodes;
