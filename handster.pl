...
our %app_map_func = (
        ...
        official_comment_times  => sub{
            ...
            my @nodes = $tree->look_down( id => 'all-reviews');
            my @reviews = $nodes[0]->find_by_attribute( 
                    class => 'product-review'
            );
            return scalar @reviews;
        },
        current_version         => sub{
            my ( $html,$app_info ) = ( shift,pop );
            my @nodes = $tree->look_down( class => 'product-title' );
...
EOF
        ...
        official_rating_times   => sub {
            ...
            my @nodes = $tree->look_down( class => 'rating-text' );
            return unless @nodes;
            if ( $nodes[0]->as_text =~ m{(\d+)\s+rating}si ){
                return $1
            }
        ...
        description             => sub{
            ...
            if( @nodes ){
                $desc = $nodes[0]->as_HTML;
                # <div class="inner-container" id="product-description">
            }else{
                ...
                $desc = $node->as_HTML;
                $desc =~ s/<b>Summary:<\/b>//sg;
            }
            $desc =~ s/<div.*?>//sg;
            $desc =~ s/<\/div>//sg;
            $desc =~ s/<b>Application description<\/b>//sg;
            $desc =~ s/<(.+?)\s+(.+?)=(.+?)>/"<".$1.">"/eg;
            $desc =~ s/\\r//g;
            $desc =~ s/\\n//g;
            $desc =~ s/\\r\\n//g;
...
sub run{
    ...
    my $page_file = 'hand_page.html';
    my $feeder_file = 'hand_feeder.html';
    my $app_info_file = 'hand_angry_sub.html';
    getstore( 'http://www.handster.com/themes_and_skins_animals_and_nature.htm',$page_file)
            unless -e $page_file;
    getstore(
    ...
    getstore(
            'http://www.handster.com/beiks_talking_englishitalian_travel_phrase_book_for_android.html',$app_info_file )
...
#    extract_app_from_feeder( undef,undef,{ web_page => $page} ,$app_list);
    $app_info->{app_url} =
        'http://www.handster.com/dragon_ball_z_super_butouden_3_dragonballz3paidv11.html';
