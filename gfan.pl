...
#		print $app_info->{apk_url};
	...
	if ($webpage =~ /$tag1="$app_name_tag">(.+)/) { #OK
		...
		($app_info->{app_name}, $app_info->{current_version}) = (split(/ /, $temp))[0, 1];
		 $app_info->{current_version} =~s/[^\d\.]//g;	
	} 	
    ...
    if ($webpage =~ /$os_tag_cn(.+)/) {
		...
		$temp =~s/[：|&nbsp;]/ /g;
		$temp =~s/^\s|\s$//g;
		my @version = split(/\s+/, $temp);
		$app_info->{system_requirement} = join(",", @version);
		($app_info->{min_os_version}, $app_info->{max_os_version}) = ($version[0], $version[$#version]);
...
#	print "@scree_shot_url, \n$count\n";
	...
	$app_info->{official_rating_stars} = getCommentMark($score_value);	
    }
...
sub getCommentMark {    
    ...
    return 0 if ( $app_star_width eq '0' || not defined $app_star_width );
