...
#$webpage =~ m!<span id="btAsinTitle" style="">(.*?)</span>!g;
    ...
    $app_info->{price}=0;
    $app_info->{price}=$1 if $webpage =~ m!<b class="priceLarge">\$([\d\.]+)</b>!; 
    $app_info->{price}=0 if $app_info->{price}=~/0\.0/;

    ##category from DB
    my %cat_info = $db_helper->get_category_from_feeder($app_info->{app_url_md5});
    if (defined($cat_info{sub_category}) and defined($category_mapping{$cat_info{sub_category}})){
        $app_info->{official_category}=$cat_info{sub_category};
        $app_info->{trustgo_category_id}=$category_mapping{$cat_info{sub_category}};
    }elsif(defined($cat_info{parent_category}) and defined($category_mapping{$cat_info{parent_category}})){
        $app_info->{official_category}=$cat_info{parent_category};
        $app_info->{trustgo_category_id}=$category_mapping{$cat_info{parent_category}};
    }else{
...
sub extract_page_list
{
    ...
    eval 
    {
        ...
        if ($node[0]->as_text =~ /of\s+([\d,]+)\s+Results/){
            ...
            warn 'fail to extract page list';
            return;
        }
        $total_apps =~s/,//g;
...
#$apps->{$1}=$app_url if $app_url=~/\/dp\/(B.*?)\//;
            $apps->{$2}=$1."?ie=UTF8" if $app_url=~/(.*?\/dp\/(B.*?))\//;
        }
        $tree = $tree->delete;
    };
